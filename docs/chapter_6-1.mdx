...
1. 함수를 새로 만들고 목적을 잘 드러낸느 이름을 붙인다. ('어떻게'가 아닌 '무엇을' 하는지가 드러나야 한다.)
...
function moreTahnFiveLateDeliveries(driver) {
...
const quantityDiscount =
  Math.max(0, order.quantity - 500) * order.itemPrice * 0.05;
const shippint = Math.min(order.quantity * order.itemPrice * 0.1, 100);
...
4.  테스트한다.
5.  기존 함수를 인라인한다.
6.  이름을 임시로 붙여뒀다면 함수 선언 바꾸기를 한 번 더 적용해서 우너래 이름으로 되돌린다.
7.  테스트한다.
...
4. 변수의 접근 범위를 제한한다.
5. 테스트한다.
6. 변수 값이 레코드라면 레코드 캠슐화하기를 적용할지 고려해본다.
