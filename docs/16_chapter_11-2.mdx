...
다만 이러한 패턴은 저자도 언급하고 있다시피, 객체는 지원하지만 일급 합수를 지원하지 않는 프로그래밍 언어에서 유용하게 쓰일 수 있다. 이를 통해 일급 함수의 기능을 지원하지 않는 언어에서도 일급 함수와 유사한 기능을 흉내낼 수 있다.

그러나 이러한 변경은 언제나 트레이드 오프를 달고 다닌다. 일반 객체에 비해 명령 객체는 보통 복잡도가 높기 마련이다. 게다가 자바스크립트에서는 일급 함수를 지원하기 때문에 진짜 쓸 일이 거의 없다. 정말 복잡한 기능을 사용할 때만 쓰도록 하자.
...
다음은 건강보험 애플리케이션에서 사용하는 점수 계산 함수다. 이 복잡한 함수를 크래스로 옮기며 리팩터링 해보겠다.
...
```js
function score(candidates, medicalExam, scoringGuide) {
  return new Score(candidates, medicalExam, scoringGuide);
}

class Scorer {
  constructor(candidates, medicalExam, scoringGuid) {
    return new Score(candidates, medicalExam, scoringGuid);
  }
...
크게 복잡하지 않읜 함수로 대체하는 리팩터링을 적용해보겠다. 가장 먼저 이 클래스를 생성하고 호출하는 코드를 함께 함수로 추출한다.
