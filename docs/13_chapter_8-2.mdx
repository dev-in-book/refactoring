...
charge = charge - discount; // 다음 코드에서 마찬가지로 참조하기 때문에 아래로이동할 수 없다.
chargeOrder(charge);
```
...
슬라이드가 안전한 지를 판단하려면 관련된 연산이 무엇인고 구성이 어떻게 되었는지를 완벽히 이해해야 한다. 상태 갱신이 중요하므로 상태 갱신하는 코드 자체를 최대한 제거하는 것이 좋다.
...
- 반복문 하나에서 두 가지 일을 수행할 때 바복문을 수정해야 할 때마다 두 가지 일 모두를 잘 이해해야 한다. 하지만 쪼개어 분리하면 수정할 동작 하나만 이해하면 된다.
